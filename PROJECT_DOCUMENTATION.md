...
**Runs on Cloudflare Workers as of August 2026** (migrated from 7 Java Spring Boot microservices for a one-week sandbox-testing deadline — see [`ARCHITECTURE.md` §1.1](./ARCHITECTURE.md#11-migration-from-the-original-java-architecture) for the migration record). Every feature described below is unchanged by that migration — it was a lift-and-shift of behavior, not a redesign, so this document didn't need a feature-by-feature rewrite, only §4 (Local development) below.
...
A 5-tier approval hierarchy — **Underwriter → Senior Underwriter → Head of Lending → COO → CEO** — each with an admin-configurable maximum approval amount (`/admin/mandates`, same admin-editable pattern as Affordability Rules, see §2.5 — both persisted in D1, surviving a redeploy, since the August 2026 Cloudflare Workers migration; they were in-memory/resets-on-restart Spring beans before that). The Decision tab shows the logged-in approver's own mandate limit; if the requested/approved amount exceeds it, **Approve is disabled outright** and the underwriter must use **Refer to Senior** instead — there is no override.
...
- **Users** (role assignment, including all 5 underwriter-hierarchy roles plus `BUSINESS_OWNER`), **FAQs**, **Branding** (logo/colors — fully theme-aware, no hardcoded brand colors), **Rules** (affordability thresholds, editable at runtime), **Mandates** (per-role approval limits, see §1.2.2), **Products** (full CRUD over every personal and business loan product — code, name, description, rate, amount/term range, min credit score, min income, max DTI, eligible risk categories, active flag — with a Personal/Business filter; `AdminProductsComponent`, mirrors the existing FAQs admin page's list+form pattern. Backend: `GET/POST/PUT/DELETE /api/products/admin*` in `ProductController`, enforcing unique `productCode` on create/update), **Email Templates** (see §1.3.1).

#### 1.3.1 Email Templates (2026-08-18)

Real outbound email, separate from and additional to the in-app notification bell (`worker/src/lib/notifications.ts`) and the on-screen OTP code — neither of those is affected by this feature. **Admin → Email Templates** lets an admin configure, per application-lifecycle event, whether an email actually goes out and what it says: an event dropdown (Submitted, Document Requested, Clarification Requested, Approved, Declined, Sent Back, Funds Released), an enabled toggle, optional **To** (blank defaults to the applicant's own email on that application) and **CC** (blank = none, comma-separated for multiple addresses), Subject, a Header banner, Body, Signature, and Footer — each supporting `{{variableName}}` placeholders (e.g. `{{applicantName}}`, `{{applicationRef}}`, `{{declineReason}}`) inserted via clickable chips scoped to what's actually available for that event. A live preview (rendered server-side with the real branding colors/logo, sample data) and a "Send test email" button (always targets the admin's own inbox, and — unlike production sends — surfaces delivery errors instead of swallowing them) let the admin verify a template before relying on it.

Delivery is via **Resend's HTTP API** (`worker/src/lib/email.ts`, the first outbound third-party integration in this Worker), requiring a `RESEND_API_KEY` secret and a `RESEND_FROM_EMAIL` var (see `wrangler.toml`). **Caveat**: until a sending domain is verified in the Resend account, real delivery only reaches the account owner's own address — the admin "Send test email" button works regardless, since it always targets the logged-in admin. Sending is best-effort and non-blocking, mirroring the existing in-app notification convention: a missing/disabled template, a missing API key, or a Resend failure is logged and swallowed, never blocking the underlying business action (approve, decline, etc).
...
- **Affordability**: a parallel `POST /api/affordability/check-business` computes **DSCR** (debt-service coverage ratio = (annual turnover − business outgoings) ÷ annual debt service) instead of DTI/HTI, with hard stops for company liquidation/director default and a soft stop below a DSCR threshold. Thresholds are currently hardcoded constants in `worker/src/lib/affordability-calc.ts` (not yet on the admin Rules page — flagged as a fast-follow, matching how personal affordability thresholds started before that page existed).
...
- **Validation**: Angular reactive form validators client-side; Zod schemas at the Worker's route boundary + a global error handler server-side. Business-rule violations (duplicate email, application not found, etc.) throw a structured `AppError`, caught centrally and returned as `{ success: false, message }`.
...
See **[`ARCHITECTURE.md` §8](./ARCHITECTURE.md#8-local-development-current--cloudflare-workers)** for the full local dev workflow (`wrangler dev` + `ng serve`) and seeded login credentials.

**Public hosting, since the August 2026 Cloudflare Workers migration**: the app is permanently live at `https://digibank-personal-loans.mohit-chimote.workers.dev` — no session to keep alive, no machine to leave running, no inactivity timeout. `npx wrangler deploy` from `worker/` ships a new version in under 20 seconds. This replaced the GitHub Codespaces hosting approach below, which was always a workaround for the Java version needing a persistent server; Workers doesn't have that problem to begin with.

**Resetting demo data**: `worker/scripts/cleanup-test-data.sql` (via `wrangler d1 execute digibank --remote --file=...`) clears all applications/users/notes/documents created during testing while leaving the seed data (staff accounts, FAQs, products, rules) untouched — safe to run before handing the sandbox to a new round of testers.

<details>
<summary>Pre-migration hosting history (reference only — GitHub Codespaces, superseded)</summary>
...
a card for identity verification and the authorization hold kept failing. Codespaces needed no
payment method at all; it was covered by the GitHub account's free monthly hours.

Known limitation this approach never fully solved: Codespaces' default machine size was tight
running 7 JVMs + MySQL simultaneously, and a Codespace stops after a period of inactivity
(session-based, not a persistent server) — fine for a scheduled live demo, never a substitute for
real always-on hosting. This is exactly the problem the Cloudflare Workers migration solved.

**Startup gotcha (root-caused 2026-06-24, fixed by `start-all.ps1`)**: the old `start-backend.ps1`
fired all 7 `mvnd spring-boot:run` builds within ~14 seconds of each other. Multiple `mvnd` builds
resolving dependencies at once contended for the same artifact lock files under
`.m2/repository/.locks`, producing a hard `BUILD FAILURE` that looked like a MySQL/Hikari
thundering-herd issue from the outside but was actually Maven, not the DB. `start-all.ps1` fixed
this by waiting for each service's port to bind before launching the next. None of this applies to
the Workers version — there's one process, not seven racing JVMs.
