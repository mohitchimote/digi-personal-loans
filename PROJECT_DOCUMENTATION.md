...
- **Known demo gap — credit score vs. business product eligibility**: business products require a minimum director credit score (6, 6, 7 respectively), but the synthetic score generated for a business application (§1.1.2) is a uniform 1–9, so roughly half of newly-created business applications will see **zero eligible products** purely because the random score happened to land below every threshold. This is the same root cause that affects personal loans far less visibly (personal products have a wider score floor of 5-7 against a similarly-distributed synthetic score, so it's rarer to lock out every product there). Tracked as a backlog item — see project memory `project_backlog_credit_score_business_financials.md` for the planned fix (likely: re-introduce a visible, demo-only credit score input so a presenter can deliberately showcase both an approved and a declined/no-product path, while keeping the field hidden in a hypothetical production build).
...
- **Backlog (not started)**: the underwriter's required-document checklist (§1.2, currently national ID/payslips/bank-statements-flavored, built for personal cases) hasn't been audited/extended for what a business case should actually require (certificate of incorporation, financial statements, business bank statements — the document *types* already exist in document-service, see §2.7, but the case-detail checklist UI itself needs business-aware required-types logic, the same way it already conditionally adds `GUARANTOR_ID`). Also see §1.4.1 for a much larger planned addition: synthetic Financial Ratios/P&L/Cashflow Analysis/Risk Grading/DSCR generated from uploaded business documents for the underwriter's Affordability tab. Both tracked in project memory `project_backlog_credit_score_business_financials.md`.

#### 1.4.1 Planned: richer Business Financials for underwriters (not started)

Today's `BusinessAffordabilityService` computes a single DSCR number from whatever the customer typed into Business Financials/Outgoings. The plan (captured 2026-06-21, not yet built) is to give underwriters a much richer financial picture — **Financial Ratios, P&L summary, Cashflow Analysis, a Risk Grade, and DSCR** — sourced from supporting documents the customer uploads (financial statements, business bank statements), not extra wizard form fields. Since there's no real OCR/document-extraction integration, the plan is to **fake the extraction** exactly the way this project already fakes Open Banking and Data Verification (§1.2.1) — generate plausible, internally-consistent synthetic figures once a document is uploaded, seeded by the application reference, and surface them as a read-only panel on the underwriter's case-detail Affordability tab. The customer-facing journey is meant to stay light: attach the documents, nothing more — no new required form fields beyond what already exists.
