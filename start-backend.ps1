...
# rule-service also goes early, before affordability-service/application-service —
# ARCHITECTURE_REVIEW_GAPS.md G4: affordability-service's rules assessment path and
# application-service's mandate-limit enforcement both call rule-service directly (a static URL,
# not Eureka lb://), so they'll fail those specific calls until it's reachable, even though both
# services still start up fine without it.
$services = @(
...
foreach ($svc in $services) {
    ...
    if ($svc.name -eq "service-registry" -or $svc.name -eq "rule-service") {
